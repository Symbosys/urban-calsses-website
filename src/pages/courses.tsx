...
const CoursesPage = () => {
  ...
  return (
    <div className={`pt-24 sm:pt-32 md:pt-36 min-h-screen transition-colors duration-500 overflow-x-hidden relative ${
      theme === 'dark' ? "bg-[#0b0f1a]" : "bg-slate-50"
    }`}>
      {/* Dynamic Background Elements - Optimized for Mobile */}
      <div className="absolute top-0 left-0 w-full h-[600px] md:h-[800px] pointer-events-none opacity-40 md:opacity-50 overflow-hidden">
        <div className={`absolute top-[-10%] left-[-20%] md:left-[-10%] w-[300px] sm:w-[500px] md:w-[800px] h-[300px] sm:h-[500px] md:h-[800px] rounded-full blur-[80px] md:blur-[150px] transition-colors duration-1000 ${
          theme === 'dark' ? "bg-blue-600/20" : "bg-blue-200/40"
        }`} />
        <div className={`absolute top-[10%] right-[-20%] md:right-[-10%] w-[250px] sm:w-[400px] md:w-[600px] h-[250px] sm:h-[400px] md:h-[600px] rounded-full blur-[80px] md:blur-[150px] transition-colors duration-1000 ${
          theme === 'dark' ? "bg-indigo-600/20" : "bg-indigo-200/40"
        }`} />
      </div>

      {/* Hero & Search Section */}
      <section className="relative py-12 sm:py-20 md:py-24 lg:pb-32">
        <div className="container mx-auto px-4 sm:px-6 relative z-10">
          ...
          >
            <div className={`inline-flex items-center gap-2 px-4 py-1.5 rounded-full mb-6 sm:mb-8 backdrop-blur-md border ${
              theme === 'dark' ? "bg-white/5 border-white/10 text-blue-400" : "bg-blue-50 border-blue-100 text-blue-600"
            }`}>
              <Sparkles size={14} className="animate-pulse" />
              <span className="text-[10px] sm:text-xs font-bold uppercase tracking-widest">Premium Learning Experience</span>
            </div>
            <h1 className={`text-3xl sm:text-5xl md:text-7xl font-black mb-4 sm:mb-6 tracking-tight leading-[1.1] transition-colors ${
            ...
            <p className={`text-base sm:text-lg md:text-xl font-medium max-w-2xl mx-auto leading-relaxed transition-colors px-2 ${
          ...
          >
            <div className={`absolute -inset-1 bg-gradient-to-r from-blue-600 to-indigo-600 rounded-2xl md:rounded-3xl blur opacity-10 sm:opacity-20 transition duration-1000 group-focus-within:opacity-40`}></div>
            <div className="relative group">
              <Search className="absolute left-5 sm:left-6 top-1/2 -translate-y-1/2 text-slate-500 group-focus-within:text-blue-500 transition-colors" size={20} />
              <input 
                type="text" 
          ...
          {/* Categories Selector - Responsive Scroll/Wrap */}
            ...
            {categories.map((cat, i) => {
              ...
              return (
                <motion.button
                  ...
                  className={`relative flex items-center gap-2 sm:gap-3 px-5 sm:px-8 py-2.5 sm:py-4 rounded-xl sm:rounded-2xl text-[11px] sm:text-sm font-bold tracking-tight transition-all active:scale-95 group overflow-hidden ${
                ...
                >
                  ...
                  {hasImageUrl ? (
                    <img src={iconObj.secure_url} alt="" className="relative z-10 w-4 h-4 sm:w-5 sm:h-5 object-contain" loading="lazy" />
                  ) : (
                    <Icon size={16} className="relative z-10 sm:size-[20px]" />
                  )}
          ...
          <AnimatePresence mode="popLayout">
            {coursesLoading ? (
               <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6 sm:gap-10">
                 {[1,2,3,4,5,6].map(i => (
                   <div key={i} className={`aspect-[4/5] rounded-[1.5rem] sm:rounded-[2.5rem] animate-pulse ${theme === 'dark' ? 'bg-white/5' : 'bg-slate-200'}`} />
                 ))}
               </div>
            ) : courses.length > 0 ? (
              <motion.div 
                ...
                {courses.map((course: any) => (
                  ...
                  >
                    <Link to={`/course/${course.id}`} className="block h-full group">
                      <div className={`relative h-full border transition-all duration-500 rounded-[1.8rem] sm:rounded-[2.5rem] overflow-hidden flex flex-col ${
                        theme === 'dark' 
                        ...
                        <div className="px-5 sm:px-8 pb-6 sm:pb-8 flex-1 flex flex-col">
                          <div className="flex justify-between items-center mb-4 sm:mb-5">
                            ...
                            <div className="flex items-center gap-1.5">
                               <Star size={12} className="text-yellow-500" fill="currentColor" strokeWidth={0} />
                               <span className={`text-[11px] sm:text-xs font-black ${theme === 'dark' ? 'text-white' : 'text-slate-900'}`}>{course._count?.reviews ? (4.5 + (course.title.length % 5) / 10).toFixed(1) : "4.8"}</span>
                               <span className="text-slate-500 text-[9px] sm:text-[10px] font-bold">({course._count?.enrollments || "12k+"})</span>
                            </div>
                          </div>

                          <h3 className={`text-lg sm:text-2xl font-black mb-4 sm:mb-6 leading-tight transition-colors group-hover:text-blue-500 ${
                            theme === 'dark' ? "text-white" : "text-slate-900"
                          }`}>
                            {course.title}
                          </h3>
                            ...
                            {(course.tags?.slice(0, 3) || ["Elite Faculty", "Study Material", "Test Series"]).map((f: any, index: number) => (
                              <div key={index} className="flex items-center gap-2.5 sm:gap-3">
                                <div className="flex items-center justify-center w-4 h-4 sm:w-5 sm:h-5 rounded-full bg-blue-500/10">
                                  <CheckCircle2 size={10} className="text-blue-500 sm:size-[12px]" />
                                </div>
                          ...
                          {/* Price & Action */}
                          <div className={`mt-auto pt-6 sm:pt-8 border-t flex items-center justify-between ${
                            theme === 'dark' ? "border-slate-800" : "border-slate-100"
                          }`}>
                             <div className="flex flex-col">
                               <div className="flex items-center gap-1.5 sm:gap-2">
                                 <span className={`text-xl sm:text-3xl font-black ${theme === 'dark' ? 'text-white' : 'text-slate-900'}`}>₹{course.price}</span>
                                 {course.discountPrice && (
                                   <span className="bg-emerald-500/10 text-emerald-500 text-[8px] sm:text-[10px] font-black px-1.5 sm:px-2 py-0.5 rounded uppercase leading-none">Save {Math.round(((course.discountPrice - course.price) / course.discountPrice) * 100)}%</span>
                                 )}
                               </div>
                               {course.discountPrice && (
                                 <span className="text-slate-500 line-through text-[11px] sm:text-sm font-bold opacity-50">₹{course.discountPrice}</span>
                               )}
                            </div>
              ...
              >
                <div className="relative inline-block mb-8 sm:mb-10">
                  ...
                  <div className={`relative w-20 h-20 sm:w-28 sm:h-28 rounded-full flex items-center justify-center shadow-inner ${
                    theme === 'dark' ? "bg-slate-800 text-slate-500" : "bg-white text-slate-400"
                  }`}>
                    <Search size={30} className="sm:size-[40px]" />
                  </div>
                </div>
                <h3 className={`text-2xl sm:text-3xl font-black mb-3 sm:mb-4 ${theme === 'dark' ? 'text-white' : 'text-slate-900'}`}>No Matches Found</h3>
                <p className="text-slate-500 font-bold max-w-xs sm:max-w-sm mx-auto text-sm sm:text-base">We couldn't find any batches matching your criteria. Try widening your filters.</p>
                <button 
               ...
               { icon: Sparkles, label: "Structured Study Plans", value: "100% Success" }
                ...
                <div key={i} className="group">
                  <div className="w-12 h-12 sm:w-16 sm:h-16 rounded-xl sm:rounded-2xl bg-blue-600/5 flex items-center justify-center mx-auto mb-4 sm:mb-6 group-hover:bg-blue-600 group-hover:text-white transition-all duration-500">
                    <item.icon size={20} className="text-blue-500 group-hover:text-white transition-colors sm:size-[28px]" />
                  </div>
                  <h4 className={`text-sm sm:text-lg font-black mb-1 sm:mb-2 ${theme === 'dark' ? 'text-white' : 'text-slate-900'}`}>{item.value}</h4>
                  <p className="text-slate-500 text-[8px] sm:text-xs font-bold uppercase tracking-widest leading-tight">{item.label}</p>
                </div>
             ))}
          </div>
        </div>
