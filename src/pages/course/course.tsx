...
import {
  ...
  useVerifyPayment,
  useCheckEnrollment,
} from "../../api/hooks/courses/purchase.hooks";
...
const CourseDetail = () => {
  ...
  const handlePurchase = async () => {
    ...
    if (isEnrolled) {
      showSuccessMessage(
        "You are already enrolled! Redirecting to course content...",
      );
  ...
  if (isLoading) {
    return (
      <div
        className={`min-h-screen flex items-center justify-center ${theme === "dark" ? "bg-[#080b14]" : "bg-slate-50"}`}
      >
  ...
  if (!course) {
    return (
      <div
        className={`min-h-screen flex flex-col items-center justify-center p-6 text-center ${theme === "dark" ? "bg-[#080b14]" : "bg-slate-50"}`}
      >
        ...
        <p className="text-slate-500 mb-8 font-medium max-w-md">
          The course you're looking for doesn't exist or has been removed from
          our catalog.
        </p>
        <button
  ...
  const quickStats = [
    {
      icon: Users,
      label: "Community",
      val: `${course._count?.enrollments || 0}+`,
    },
    {
      icon: Clock,
      label: "Experience",
      val: `${course.durationMonths} Months`,
    },
    {
      icon: PlayCircle,
      label: "Learning Hours",
      val: `${course.totalLessons * 15}+`,
    },
  ...
  return (
    <div
      className={`pt-20 min-h-screen transition-all duration-700 overflow-x-hidden ${
    ...
    >
      ...
      <div className="fixed top-0 left-0 w-full h-full pointer-events-none overflow-hidden z-0">
        <div
          className={`absolute top-[-20%] right-[-10%] w-[80vw] h-[80vw] rounded-full blur-[150px] transition-all duration-1000 ${
            theme === "dark" ? "bg-blue-600/5" : "bg-blue-100/30"
          }`}
        />
        <div
          className={`absolute bottom-[-10%] left-[-10%] w-[60vw] h-[60vw] rounded-full blur-[150px] transition-all duration-1000 ${
        ...
        <div className="grid lg:grid-cols-12 gap-12 xl:gap-20 items-start">
          ...
          <div className="lg:col-span-8">
            ...
            >
              ...
              <h1
                className={`text-4xl md:text-6xl xl:text-7xl font-black mb-10 leading-[1.05] tracking-tight transition-colors ${
                  theme === "dark" ? "text-white" : "text-slate-900"
                }`}
              >
                {course.title.split(" ").map((word: string, i: number) => (
                  <span key={i} className={i % 3 === 2 ? "text-blue-600" : ""}>
                    {" "}
                    {word}{" "}
                  </span>
                ))}
              ...
              <p
                className={`text-lg md:text-xl mb-12 leading-relaxed max-w-3xl transition-colors font-medium ${
              ...
              <div className="mb-20">
                ...
                <AnimatePresence mode="wait">
                  ...
                  >
                    {activeTab === "Overview" && (
                      <div
                        className={`p-10 rounded-[3rem] border backdrop-blur-3xl transition-all ${
                          theme === "dark"
                            ? "bg-white/[0.01] border-white/[0.05]"
                            : "bg-white border-slate-200 shadow-xl shadow-blue-500/5"
                        }`}
                      >
                        <h3
                          className={`text-2xl font-black mb-10 ${theme === "dark" ? "text-white" : "text-slate-900"}`}
                        >
                          {course.title}: Deep Dive
                        </h3>
                        <div className="prose prose-lg prose-slate max-w-none mb-12">
                          ...
                          >
                            {course.description ||
                              "In-depth professional training designed to take you from foundational understanding to advanced mastery. Our curriculum is structured to follow industry best practices, ensuring you're ready for real-world challenges."}
                          </p>
                        </div>
                          ...
                          {[
                            {
                              ...
                              d: "Proprietary methodology to master topics 2x faster than traditional methods.",
                            },
                            {
                              icon: ShieldCheck,
                              t: "Industry Standards",
                              d: "Curriculum designed with Fortune 500 engineering leaders.",
                            },
                            {
                              icon: Users,
                              t: "Collaborative Labs",
                              d: "Work on real projects with a high-performance peer community.",
                            },
                            {
                              icon: BarChart3,
                              t: "Career Analytics",
                              d: "Deep insights into your learning patterns and job readiness.",
                            },
                          ].map((feat: any, i: number) => (
                            <div key={i} className="flex gap-5 group">
                              <div
                                className={`w-12 h-12 rounded-2xl flex items-center justify-center shrink-0 transition-all ${theme === "dark" ? "bg-white/5 group-hover:bg-blue-600/20" : "bg-slate-50 group-hover:bg-blue-50"}`}
                              >
                                <feat.icon
                                  size={22}
                              ...
                              <div>
                                <h5
                                  className={`font-black text-lg mb-2 ${theme === "dark" ? "text-slate-100" : "text-slate-800"}`}
                                >
                                  {feat.t}
                                </h5>
                                <p
                                  className={`text-sm leading-relaxed font-medium ${theme === "dark" ? "text-slate-500" : "text-slate-500"}`}
                                >
                                  {feat.d}
                                </p>
                              </div>
                    ...
                    {activeTab === "Curriculum" && (
                      <div className="space-y-4">
                        {(course.subjects || []).length > 0 ? (
                                ...
                                <div className="flex items-center gap-6">
                                  ...
                                  <div className="text-left">
                                    ...
                                    <p className="text-slate-500 text-[10px] font-black uppercase tracking-widest mt-1">
                                      {subject.sections?.length || 0} Dynamic
                                      Modules
                                    </p>
                                  </div>
                                </div>
                                <div
                                  className={`w-10 h-10 rounded-full border border-white/10 flex items-center justify-center transition-transform duration-500 ${expandedModule === i ? "rotate-180 bg-blue-600 border-blue-600 text-white" : "text-slate-500"}`}
                                >
                                  <ChevronRight
                                    size={18}
                                    className="rotate-90"
                                  ...
                                  >
                                    <div className="p-8 pt-0 grid md:grid-cols-2 gap-4">
                                      {subject.sections?.map((section: any) => (
                                        <div
                                          key={section.id}
                                          className={`flex items-center gap-4 p-5 rounded-2xl border group/sec transition-all ${
                                            theme === "dark"
                                              ? "bg-white/5 border-white/5 hover:border-blue-500/30"
                                              : "bg-slate-50 border-slate-200/50 hover:border-blue-500/30"
                                          }`}
                                        >
                                          <div className="w-8 h-8 rounded-xl bg-blue-600/10 flex items-center justify-center group-hover/sec:bg-blue-600 transition-all">
                                            <PlayCircle
                                              size={14}
                                              className="text-blue-600 group-hover/sec:text-white"
                                            />
                                          </div>
                                          <span
                          ...
                          <div
                            className={`p-20 text-center rounded-[3rem] border border-dashed ${theme === "dark" ? "border-white/10" : "border-slate-200"}`}
                          >
                            <p className="text-slate-500 font-black uppercase tracking-widest text-sm">
                              Detailed Curriculum Loading...
                            </p>
                          </div>
                        )}
                      </div>
                          ...
                          (inst: any, i: number) => (
                            ...
                            >
                              <div className="w-20 h-20 rounded-3xl overflow-hidden shadow-xl shrink-0">
                                <img
                                  src={
                                    inst.instructor.avatar?.secure_url ||
                                    `https://api.dicebear.com/7.x/avataaars/svg?seed=${inst.instructor.name}`
                                  }
                              ...
                              <div>
                                <h4
                                  className={`text-xl font-black mb-1 ${theme === "dark" ? "text-white" : "text-slate-900"}`}
                                >
                                  {inst.instructor.name}
                                </h4>
                                <p className="text-blue-500 text-xs font-black uppercase tracking-widest mb-3">
                                  Senior Instructor
                                </p>
                    ...
                    {activeTab === "Reviews" && (
                      ...
                      >
                        ...
                        <p className="text-slate-500 font-black uppercase tracking-widest text-sm">
                          Real Student Feedback Coming Soon
                        </p>
                      </div>
                    )}
                  </motion.div>
            ...
            >
              ...
              <div className="p-8">
                <div className="flex items-center gap-4 mb-10">
                  <div className="flex flex-col">
                    ...
                    {course.discountPrice && (
                      <span className="text-slate-500 line-through text-base font-bold opacity-50">
                        ₹{course.discountPrice}
                      </span>
                    )}
                  </div>
                  <div className="ml-auto px-4 py-2 bg-emerald-500/10 text-emerald-500 text-[10px] font-black rounded-xl border border-emerald-500/20 uppercase tracking-widest">
                    {course.discountPrice
                      ? `${Math.round(((course.discountPrice - course.price) / course.discountPrice) * 100)}% Savings`
                      : "Elite Enrollment"}
                  </div>
                </div>
                ...
                <div className="bg-slate-500/5 rounded-3xl p-6 border border-white/5">
                  <h5
                    className={`text-[10px] font-black uppercase tracking-[0.2em] mb-4 ${theme === "dark" ? "text-slate-500" : "text-slate-400"}`}
                  >
                    What's Included
                  </h5>
                  <div className="space-y-4">
                ...
                <div className="mt-6 space-y-4">
                  ...
                  <button
                    className={`w-full py-5 rounded-2xl font-black text-[10px] uppercase tracking-widest border transition-all active:scale-95 flex items-center justify-center gap-3 ${
                      theme === "dark"
                        ? "bg-white/5 border-white/5 text-slate-300 hover:opacity-80"
